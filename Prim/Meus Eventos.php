...
$SQL = "SELECT TITULO_EVENTO,DATA_EVENTO,HORA_EVENTO,ID_EVENTO,ING_VENDIDOS_EVENTO FROM EVENTO WHERE EMAIL_USER='$email'";
    ...
    <div class="wrapper d-flex align-items-stretch">
      ...
      <div class="row align-items-start">
        <?php while($mostraEventos = mysqli_fetch_array($consulta)){ ?>    
          ...
          <?php if($mostraEventos[4] == 0){ ?>
          <form method="post" action="Backend/deletaServico.php">
            <button value="<?php echo "$mostraServico[3]";?>" type="submit" name="ID" style="background:none; border:none;float:right">
            <img src="Imgs/excluir.png" width ="45px"alt=""></button>
          </form>             
          <?php } ?>

          <form method="post" action="EventoD.php" >
            <button  value="<?php echo $mostraEventos[3]?>" name="event" type="submit" style="background:none; border:none;">
            <img src="Imgs/Card.jpg" class="card-img-top" alt="..."></button>
          </form>

              <div class="card-body">
                <h5 class="card-titulo"><Strong> <?php print "$mostraEventos[0]"; ?></Strong></h5>
                <p class="card-text"><?php print date("d/m/Y",strtotime($mostraEventos[1]));
                                  print str_repeat("&nbsp;", 9);
                                  print("Horário: ");
                                  print date("H:i",strtotime($mostraEventos[2]));?></p>
                <p style="color:green;">Vendido: <?php echo $mostraEventos[4]?></p>
              </div>                    
            </div>
        <?php } ?>
      </div>
